...
if (isset($_POST['transaction'])) {
    ...
    if ($stmt->execute()) {
        ...
        if ($check && $check->num_rows > 0) {
            ...
            $transaction_code = uniqid('TXN_'); // Generate a unique transaction code
            $trans_stmt = $conn->prepare("INSERT INTO transaction (supply_id, quantity, action, transaction_code) VALUES (?, ?, ?, ?)");
            $trans_stmt->bind_param("iiss", $supply_id, $quantity, $action, $transaction_code);
            $trans_stmt->execute();
...
if (isset($_POST['delete_supply'])) {
    ...
    if ($check && $check->num_rows > 0) {
        ...
        if ($quantity >= 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO notifications (message, user_name) VALUES (?, ?)");
            $stmt->bind_param("ss", $note, $user_name);
            $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        $stmt = $conn->prepare("INSERT INTO supply_notifs (message, status, user_name) VALUES (?, 'unread', ?)");
        $stmt->bind_param("ss", $note, $user_name);
        $stmt->execute();
