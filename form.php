...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check username and password exactly as input (plain text)
    $stmt = $pdo->prepare("SELECT * FROM users WHERE username = ? AND password = ?");
    $stmt->execute([$username, $inputPassword]);
    $user = $stmt->fetch();
    ...
    if ($user) {
        ...
        header('Location: /Project/index.php?error=1');
        exit;
    }
}
?>
    ...
    <head>
        ...
        <style>
            ...
            .bg-container::before {
                ...
                width: 100%;
                height: 100%;
                background: rgba(30, 30, 60, 0.6); /* Lighter dark overlay for better visibility */
